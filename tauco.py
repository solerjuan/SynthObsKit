...
def tauco(dens, temperature, vlos, pos, vsize=100, dv=1.0, selfabsorption=True, mincodens=500.0, maxcot=50.0, minhit=0.0, maxcnmt=200.0, minwnmt=5000., maxwnmt=8500.):
   ...
   nCOtonH=7E-05 # 25% of the carbon is in CO
   ...
   #coefCO=46*(1e-3*u.cm**3)*(Lcloud.cgs/(1e19*u.cm))*((2*u.km/u.s)/vtot) # [cgs] (Draine 2011 Eq. 19.19 p223)
   lambda10=0.26*u.cm
   A10=7.203e-08/u.s
   coef_abso_CO=A10*(lambda10**3)/(8*np.pi)
   ...
   for i in range(0,nb):
      ...
      #B0/k_B=2.77 K, from Draine, 2011 p. 222-224.

      zco=np.sqrt(1+(temperature[i]/2.77)**2)
      tau_cell_co=nCOtonH*dens_co[i]*dx[i]*p_co*(1-np.exp(-2*2.77/temperature[i]))*coef_abso_CO.value/zco
